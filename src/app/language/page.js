...
export default function LanguagePage() {
  ...
  const [searchCategory, setSearchCategory] = useState("구분");
  ...
  const handleRegister = async () => {
    ...
    const selectedLanguages = languages.filter(
      (lang) => selectedIds.includes(lang.id) && lang.isEditing
    );
    ...
    try {
      ...
      setSelectedIds(
        selectedIds.filter(
          (id) => !selectedLanguages.some((lang) => lang.id === id)
        )
      );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // 구분이 선택되었을 경우 category 파라미터 추가
      if (searchCategory !== "구분") {
        params.append("category", categoryMap[searchCategory]);
      }

      // 검색어가 있을 경우 keyword 파라미터 추가
      if (searchText) {
        params.append("keyword", searchText);
      }

      const response = await fetch(`${apiUrl}/api/languages/search?${params}`);
      ...
      const formattedLanguages = data.map((lang, index) => ({
        id: lang.id || index,
        category: reverseCategoryMap[lang.category] || lang.category,
        korean: lang.koName || "",
        english: lang.enName || "",
        chinese: lang.chName || "",
        isEditing: false,
        ...
        <tbody>
          {languages.map((lang) => (
            <tr
              key={lang.id}
              style={{
                backgroundColor: selectedIds.includes(lang.id)
                  ? "#e3f2fd"
                  : "white",
              ...
              <td style={{ padding: "12px", border: "1px solid #ddd" }}>
                {lang.isEditing ? (
                  <select
                    value={lang.category}
                    onChange={(e) =>
                      handleFieldChange(lang.id, "category", e.target.value)
                    }
                    style={{
                      width: "100%",
                      padding: "8px",
              ...
              <td style={{ padding: "12px", border: "1px solid #ddd" }}>
                {lang.isEditing ? (
                  <input
                    ...
                    value={lang.korean}
                    onChange={(e) =>
                      handleFieldChange(lang.id, "korean", e.target.value)
                    }
                    placeholder="한국어 입력"
                    style={{
                      width: "100%",
              ...
              <td style={{ padding: "12px", border: "1px solid #ddd" }}>
                {lang.isEditing ? (
                  <input
                    ...
                    value={lang.english}
                    onChange={(e) =>
                      handleFieldChange(lang.id, "english", e.target.value)
                    }
                    placeholder="영문 입력"
                    style={{
                      width: "100%",
              ...
              <td style={{ padding: "12px", border: "1px solid #ddd" }}>
                {lang.isEditing ? (
                  <input
                    ...
                    value={lang.chinese}
                    onChange={(e) =>
                      handleFieldChange(lang.id, "chinese", e.target.value)
                    }
                    placeholder="중문 입력"
                    style={{
                      width: "100%",
